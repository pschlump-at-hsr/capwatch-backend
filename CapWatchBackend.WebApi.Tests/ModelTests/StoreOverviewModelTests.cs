...
namespace CapWatchBackend.WebApi.Tests.ModelTests {
  public class StoreOverviewModelTests {
    private readonly IMapper _mapper;

    public StoreOverviewModelTests() {
    ...
    public void TestMapper(string id, string name, string street, string zipCode, string city, int maxCapacity, int currentCapacity, string logo, string secret, string typeId, string typeDescription) {
      ...
      storeOverview.Street.Should().Be(street);
      storeOverview.ZipCode.Should().Be(zipCode);
      storeOverview.City.Should().Be(city);
      storeOverview.MaxCapacity.Should().Be(maxCapacity);
      storeOverview.CurrentCapacity.Should().Be(currentCapacity);
      storeOverview.Logo.Should().BeEquivalentTo(Encoding.UTF8.GetBytes(logo));
      storeOverview.StoreType.Id.Should().Be(typeId);
      storeOverview.StoreType.Description.Should().Be(typeDescription);
    }
  }
}
